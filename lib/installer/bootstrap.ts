/**
 * Bootstrap da instância AgentsFlow.
 * Configura settings iniciais após migrations.
 *
 * AgentsFlow usa MASTER_PASSWORD para auth, não Supabase Auth.
 ...
 * AgentsFlow usa MASTER_PASSWORD via env var, não Supabase Auth.
...
export async function bootstrapInstance({
  ...
  const companyName = adminName?.trim() || emailNorm.split('@')[0] || 'AgentsFlow';
