...
function buildSupportPacketText(params: {
	...
	const lines: string[] = []
	lines.push(`AgentsFlow · Support Packet · ${new Date().toLocaleString('pt-BR')}`)
...
function buildMissingPermissionsSteps(params: { objectLabel: string; objectId: string }) {
	return [
		...
		'No app AgentsFlow: cole esse token e os IDs corretos em Ajustes → Credenciais WhatsApp.',
...
function buildMissingScopesSteps(missing: string[]) {
	...
	return [
		...
		'No AgentsFlow: atualize o token em Ajustes → Credenciais WhatsApp e rode o diagnóstico novamente.',
	]
}
	...
	const nextSteps = isMissingPermissionsOrWrongId
		...
		: isTokenInvalid
			? [
				...
				'Atualize as credenciais no AgentsFlow e tente novamente.',
			]
			: [
...
function buildReportText(
	...
	lines.push(`AgentsFlow · Diagnóstico Meta/WhatsApp · ${new Date().toLocaleString('pt-BR')}`)
